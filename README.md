...
Нямам счетоводен опит, така че използвайтe информацията на свой риск :)
...
- Европейските правила за ДДС - https://taxation-customs.ec.europa.eu/taxation/vat/vat-directive/place-taxation_en?prefLang=en#supply-of-services
...
Има съдебно решение (https://legalacts.justice.bg/Search/GetActContentByActId?actId=DPYdyaKXTiVD2rOE9HLtgg%3D%3D), в което съда приема, че DocuSign няма силата на квалифициран електронен подпис, тъй като няма възможност за проверка на сертификатите. Но, ако страните предварително са се уговорили и съгласили DocuSign да има силата на собственоръчно подписан документ, тогава няма проблем. Тоест, ако ползвате DocuSign е хубаво да имате и един документ с клиента, в който изрично и двете страни се съгласявате, че всякаква следваща документация подписана с DocuSign ще има силата на собственоръчно подписан документ. Алтернативно, може да искате договора да има едно копие и с мокър подпис, който да ви сканират и изпратят, и вие да го подпишете собственоръчно от ваша страна. Така ако случайно ви го поискат като доказателство, да има минимален риск от проблеми.
...
При мен, след като свалих файла исках да го прегледам, но имаше проблем с визуализацията. PDF-а изглеждаше като счупен. Оказа се че просто viewer-a от браузъра не може да го зареди правилно. След като го отворих в Desktop версията на Adobe Acrobat се оказа, че файла е наред. 
...
За декларацията за НАП трябва вече да имате издаден Булстат номер, тъй като трябва да го попълните в декларацията.
...
![Услуга Регистрация Самоосигуряващо се лице](Регистрация%20Самоосигуряващо%20се%20лице/НАП_Услуга_Декларация_самоосигуряващо_се_лице.png)  

Попълвате си данните (включително ЕИК в полето "Идентификационен номер на ЮЛ/ЕТ") и подавате.

![Декларация Регистрация Самоосигуряващо се лице](Регистрация%20Самоосигуряващо%20се%20лице/НАП_Декларация_самоосигуряващо_се_лице.png)

От подаването на декларацията до приемането на регистрацията от НАП отне 30 минути.
...
- Ако клиентът ви е чуждестранна фирма от ЕС, сте задължени още преди първата фактура да бъдете регистрирани по ДДС по чл.97а, ал.2. Важна особеност в този случай (прочетете ЗДДС и [Европейските правила](https://europa.eu/youreurope/business/finance-and-tax/vat/cross-border-vat/index_en.htm#withintheeusellservicesanotherbusiness-1)), е че ДДС се плаща в страната, в която се извършва услугата, а при консултантски услуги се счита че услугата се извършва в страната на клиента. Тоест клиента плаща ДДС в собствената си страна, а вие НЕ начислявате ДДС във вашите фактури, които издавате. Използва се режима за обратно начисление на ДДС, и това се отбелязва във фактурата като я издавате.

Важно: След регистрация по ДДС сте задължени всеки месец до 14то число, да подавате декларации за предходния месец. Препоръчвам счетоводител да се занимава с тази дейност. 

Друга важна особеност при чуждестранни клиенти, е че трябва да пазите счетоводна документация на Български - тоест, издавайте фактурите на два езика, Български за пред НАП (ако ви направят проверка) и на Английски за клиента.
...
![Услуга регистрация ЗДДС](Регистрация%20ЗДДС/НАП_Услуга_регистрация_ЗДДС.png)

Как изглежда бланката за попълване (примера е конкретно за моя случай по чл.97а ал.2):

![Регистрация ЗДДС](Регистрация%20ЗДДС/НАП_Заявление_регистрация_ЗДДС.png)

Докато нормалната регистрация по ДДС от НАП имат срок 14 дни за да ви регистрират, при чл.97а ал.2 срокът е 3 дни. Тоест процеса се задвижва бързо, и трябва да сте готови да реагирате с данните, които може да ви поискат.

С мен се свързаха от НАП и изискаха следната информация:
...
- Декларация, че за периода на проверката която ще ви направят, не сте извършвали търговска дейност.

Бланките за тези декларации съм ги качил в папката [Регистрация ЗДДС](https://github.com/idafchev/Registration-as-Freelance-in-Bulgaria/tree/main/%D0%A0%D0%B5%D0%B3%D0%B8%D1%81%D1%82%D1%80%D0%B0%D1%86%D0%B8%D1%8F%20%D0%97%D0%94%D0%94%D0%A1)

На края на втория ден от подаването на заявката за регистрация бях регистриран по ЗДДС чл.97а ал.2.

След регистрацията мисля че отне около 1 ден, за да моя ДДС номер да бъде откриваем в европейската система VIES. Може да направите проверка на следния адрес: https://ec.europa.eu/taxation_customs/vies/#/vat-validation

Вашия ДДС номер е идентичен с ЕИК номера, но с префикс "BG".
